...
#        fig = plt.close()
        ...
        for si, segment in  enumerate(segments):
            ...
            #print('analysing the segment ',segment)
            analog_signals = segment.analogsignals
            subplot_index = np.arange(1,len(segments)*len(analog_signals)+1).reshape([len(segments),len(analog_signals)]).transpose().ravel()
            #print(analog_signals)
            for ti, trace in enumerate(analog_signals):
                ax1 = fig1.add_subplot(len(analog_signals),len(segments),subplot_index[sub_plt_no])
                sub_plt_no +=1
                TI = 100001*fI+ti
                itr_no += 1
                v = trace
...
#                     print('Could not be plotted:', e)
        ...
        plt.suptitle("Readings from 3 different channels(subplot titlle:channel Id)", size=16)
