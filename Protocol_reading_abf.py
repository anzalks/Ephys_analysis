...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov  7 17:11:27 2019
...
parser.add_argument('--file_path', '-f',required=True, type=str, help=' Input the file path of the folder containing .dat files for analysis ')
args = vars(parser.parse_args())
#set the directory path with files to read
folder_to_read = args['file_path']
...
files = glob.glob(folder_to_read + '/*.abf', recursive=True)
...
#            print (file_name)
    ...
    segments = nio.AxonIO(file).read_block().segments
    # segments = ()
    iteration_number = 0
    segment_number = 0
    Vm_trail = []
...
#                        print(threshold_state)
                if threshold_state == 0 and len(neuron_threshold_v) >=1:
                    ...
                    trough_v = Trace_with_features.spike_feature("trough_v")
                    trough_t = Trace_with_features.spike_feature("trough_t")
#                            downstroke = 
                    plt.plot(neuron_threshold_t,neuron_threshold_v,'o', color ='k',label = 'threshold voltage')
                    plt.plot(trough_t,trough_v,'o', color ='r',label = 'trough_v')
#                            print(neuron_threshold_t,neuron_threshold_v)
